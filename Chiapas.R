...
attach(ConcChiapas) #this is for not writing the name of the data frame and the $ ever time

#Sort Conc according to INGCOR, FOLIOVIV, FOLIOHOG
ConcChiapas<- orderBy (~+ing_cor+folioviv+foliohog, data=ConcChiapas) #this give us the households sorted by total income

#Adding the values of the expansion FACTOR. The sum is 34 million, which is the number of households in the country.
tot_hogares<-sum(ConcChiapas$factor,to.data.frame=TRUE)
...
ConcChiapas$tam_dec<-tam_dec

# Creating Deciles of Income 

ConcChiapas$MAXT<-ConcChiapas$ing_cor #vamos a crear en esta base la variable MAXT que es una copia de la columna de ingresos

ConcChiapas<-ConcChiapas[with(ConcChiapas, order(rank(MAXT))),]  #lo que hicimos aqu? fue reordenar la base con respecto a MAXT. Cosa que en realidad, ya estaba.

ConcChiapas$ACUMULA<-cumsum(ConcChiapas$factor) #aqu? creamos una variable de suma acumulada del FACTOR de viviendas.
...
for(i in 1:9)
{
  a1<-ConcChiapas[dim(ConcChiapas[ConcChiapas$ACUMULA<tam_dec*i,])[1]+1,]$factor
  ConcChiapas<-rbind(ConcChiapas[1:(dim(ConcChiapas[ConcChiapas$ACUMULA<tam_dec*i,])[1]+1),],
                             ConcChiapas[(dim(ConcChiapas[ConcChiapas$ACUMULA<tam_dec*i,])[1]+1):dim(ConcChiapas[1])[1],])
  
  b1<-tam_dec*i-ConcChiapas[dim(ConcChiapas[ConcChiapas$ACUMULA<tam_dec*i,])[1],]$ACUMULA
  ConcChiapas[(dim(ConcChiapas[ConcChiapas$ACUMULA<tam_dec*i,])[1]+1),]$factor<-b1
  ConcChiapas[(dim(ConcChiapas[ConcChiapas$ACUMULA<tam_dec*i,])[1]+2),]$factor<-(a1-b1)
}

#aqu? estamos creando otra variable de suma acumulada del n?mero de hogares
ConcChiapas$ACUMULA2<-cumsum(ConcChiapas$factor)
...
ConcChiapas[(ConcChiapas$ACUMULA2<=tam_dec),]$DECIL<-1
...
for(i in 1:9)
{
  ConcChiapas[((ConcChiapas$ACUMULA2>tam_dec*i)&(ConcChiapas$ACUMULA2<=tam_dec*(i+1))),]$DECIL<-(i+1)
}

# a lo que le qued? cero (que es la ?ltima observaci?n), ponle el decil 10
ConcChiapas[ConcChiapas$DECIL%in%"0",]$DECIL<-10

setwd("C:/Users/Erick/Dropbox/GIC/GITHUB2018/GIC/Chiapas")
write.dbf(ConcChiapas,file="ConcChiapas2010.dbf")
...
setwd("C:/Users/Erick/Dropbox/GIC/GITHUB2018/GIC/Chiapas")
write.dbf(ConcChiapas,file="ConcChiapas2018.dbf")
...
#reading the data
setwd("C:/Users/Erick/Dropbox/GIC/GITHUB2018/GIC/Chiapas")
Conc2010<-read.dbf("ConcChiapas2010.dbf",as.is = T)
...
write.dbf(c_DECIL_ES,file = "Chiapas por fuente por DECIL estimaciones 2010.dbf")
write.dbf(c_DECIL_SE,file = "Chiapas por fuente por DECIL errores standard 2010.dbf")
write.dbf(c_DECIL_CV,file = "Chiapas por fuente por DECIL CV 2010.dbf")
write.dbf(c_DECIL_LI,file = "Chiapas por fuente por DECIL LI 2010.dbf")
write.dbf(c_DECIL_ES,file = "Chiapas por fuente por DECIL LS 2010.dbf")
...
write.dbf(c_DECIL_ES,file = "Chiapas INDIGENA por fuente por DECIL estimaciones 2010.dbf")
write.dbf(c_DECIL_SE,file = "Chiapas INDIGENA por fuente por DECIL errores standard 2010.dbf")
write.dbf(c_DECIL_CV,file = "Chiapas INDIGENA por fuente por DECIL CV 2010.dbf")
write.dbf(c_DECIL_LI,file = "Chiapas INDIGENA por fuente por DECIL LI 2010.dbf")
write.dbf(c_DECIL_ES,file = "Chiapas INDIGENA por fuente por DECIL LS 2010.dbf")
...
write.dbf(c_DECIL_ES,file = "Chiapas NO por fuente por DECIL estimaciones 2010.dbf")
write.dbf(c_DECIL_SE,file = "Chiapas NO por fuente por DECIL errores standard 2010.dbf")
write.dbf(c_DECIL_CV,file = "Chiapas NO por fuente por DECIL CV 2010.dbf")
write.dbf(c_DECIL_LI,file = "Chiapas NO por fuente por DECIL LI 2010.dbf")
write.dbf(c_DECIL_ES,file = "Chiapas NO por fuente por DECIL LS 2010.dbf")
...
#reading the data
setwd("C:/Users/Erick/Dropbox/GIC/GITHUB2018/GIC/Chiapas")
Conc2018<-read.dbf("ConcChiapas2018.dbf",as.is = T)
...
write.dbf(c_DECIL_ES,file = "Chiapas Ingresos por fuente por DECIL estimaciones 2018.dbf")
write.dbf(c_DECIL_SE,file = "Chiapas Ingresos por fuente por DECIL errores standard 2018.dbf")
write.dbf(c_DECIL_CV,file = "Chiapas Ingresos por fuente por DECIL CV 2018.dbf")
write.dbf(c_DECIL_LI,file = "Chiapas Ingresos por fuente por DECIL LI 2018.dbf")
write.dbf(c_DECIL_ES,file = "Chiapas Ingresos por fuente por DECIL LS 2018.dbf")
...
write.dbf(c_DECIL_ES,file = "Chiapas INDIGENA Ingresos por fuente por DECIL estimaciones 2018.dbf")
write.dbf(c_DECIL_SE,file = "Chiapas INDIGENA Ingresos por fuente por DECIL errores standard 2018.dbf")
write.dbf(c_DECIL_CV,file = "Chiapas INDIGENA Ingresos por fuente por DECIL CV 2018.dbf")
write.dbf(c_DECIL_LI,file = "Chiapas INDIGENA Ingresos por fuente por DECIL LI 2018.dbf")
write.dbf(c_DECIL_ES,file = "Chiapas INDIGENA Ingresos por fuente por DECIL LS 2018.dbf")
...
write.dbf(c_DECIL_ES,file = "Chiapas NO Ingresos por fuente por DECIL estimaciones 2018.dbf")
write.dbf(c_DECIL_SE,file = "Chiapas NO Ingresos por fuente por DECIL errores standard 2018.dbf")
write.dbf(c_DECIL_CV,file = "Chiapas NO Ingresos por fuente por DECIL CV 2018.dbf")
write.dbf(c_DECIL_LI,file = "Chiapas NO Ingresos por fuente por DECIL LI 2018.dbf")
write.dbf(c_DECIL_ES,file = "Chiapas NO Ingresos por fuente por DECIL LS 2018.dbf")
...
Deciles2010<-read.dbf("Chiapas por fuente por DECIL estimaciones 2010.dbf")
names(Deciles2010)[1]<-c("ingcor2010")

Deciles2018<-read.dbf("Chiapas Ingresos por fuente por DECIL estimaciones 2018.dbf")
...
deciles2010No<-read.dbf("Chiapas NO por fuente por DECIL estimaciones 2010.dbf")
names(deciles2010No)[1]<-c("No2010")

deciles2018No<-read.dbf("Chiapas NO Ingresos por fuente por DECIL estimaciones 2018.dbf")
...
deciles2010indigena<-read.dbf("Chiapas INDIGENA por fuente por DECIL estimaciones 2010.dbf")
names(deciles2010indigena)[1]<-c("Indigena2010")

deciles2018indigena<-read.dbf("Chiapas INDIGENA Ingresos por fuente por DECIL estimaciones 2018.dbf")
